...
<p>Đăng nhập tài khoản Imou (chỉ chạy local, không lưu gì ra đĩa).</p>
...
<button>Đăng nhập</button></form>{err}"""

CAPTCHA = """<form method=post action=captcha>
<p>Nhập mã captcha trong ảnh:</p>
<img src="data:image/jpeg;base64,{img}"><br>
<label>Mã<input name=code required autofocus></label>
<button>Xác nhận</button></form>{err}"""


def render_devices(login: dict, devs: dict) -> str:
    d = (devs.get("data") or {})
    items = d.get("deviceList") or []
    rows = ["<tr><th>Tên</th><th>Serial (deviceId)</th><th>Model</th><th>Role</th></tr>"]
    cams = []
    for it in items:
        did = it.get("deviceId", "")
    ...
    info = f"<p>Đăng nhập OK — <b>{html.escape(ld.get('nickname',''))}</b> ({html.escape(ld.get('email',''))})</p>"
    yaml = "cameras:\n" + "\n".join(cams) if cams else "(không có thiết bị)"
    return (info + "<table>" + "".join(rows) + "</table>"
            + "<h3>Dán vào add-on (điền password camera):</h3><pre>" + html.escape(yaml) + "</pre>")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/login":
            ...
            return self._send(FORM.format(email=html.escape(email),
                              err=f'<p class=err>Lỗi {code}: {html.escape(str(r.get("msg") or r))}</p>'))
        if self.path == "/captcha":
            f = self._form()
            v = verify_captcha(STATE["email"], STATE["password"], STATE["captcha"], f.get("code", ""))
            if v.get("code") != 0:
                return self._send(CAPTCHA.format(img="", err=f'<p class=err>Captcha sai: {html.escape(str(v.get("msg")))}</p>'))
            r = get_token(STATE["email"], STATE["password"])
            if r.get("code") != 0:
                return self._send(FORM.format(email=html.escape(STATE["email"]),
                                  err=f'<p class=err>GetToken lỗi {r.get("code")}</p>'))
            STATE["token_data"] = r["data"]
            return self._finish()
    ...
    def _finish(self):
        ...
        login = do_login(td)
        if login.get("code") != 0:
            return self._send(FORM.format(email="", err=f'<p class=err>Login lỗi {login.get("code")}: {html.escape(str(login.get("msg")))}</p>'))
