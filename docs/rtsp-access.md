...
```text
192.168.2.20 channel=1 subtype=0 video:h264:1920x1080,audio:aac
192.168.2.20 channel=1 subtype=1 video:h264:640x360,audio:aac
192.168.2.21 channel=1 subtype=0 video:hevc:2304x1296,audio:aac
192.168.2.21 channel=1 subtype=1 video:h264:640x352,audio:aac
192.168.2.22 channel=1 subtype=0 video:hevc:2304x1296,audio:aac
192.168.2.22 channel=1 subtype=1 video:h264:640x352,audio:aac
```
...
```text
192.168.2.20 indoor room, not "Cam sân"
192.168.2.21 high-resolution Imou feed, currently pointed at curtain/wall
192.168.2.22 high-resolution Imou feed, outdoor/city view
```

`Cam sân` in cloud inventory is `IPC-PS70F-10M0` with two app channels:
`PT Lens` and `Fixed Lens`. It is not on the current LAN; it is a remote camera.
Do not map it to `192.168.2.20`, `192.168.2.21`, or `192.168.2.22`. Those are
...
IMOU_RTSP_USER=admin IMOU_RTSP_PASSWORD='<password>' \
  scripts/imou_probe_rtsp.py --subnet 192.168.2.0/24
```
...
```bash
IMOU_RTSP_PASSWORD='<password>' scripts/imou_xav_bridge.py --serial R5CR618NHTA --auto \
...
```bash
IMOU_RTSP_PASSWORD='<password>' scripts/imou_xav_bridge.py --serial R5CR618NHTA --auto \
  | ffmpeg -f dhav -i pipe:0 -c copy -f rtsp rtsp://127.0.0.1:8554/cam_san
```
...
Live test against remote `Cam sân` through the S21 app/P2P tunnel:

```text
phone-local XAV port: 30006
adb-forwarded host port: 42006
