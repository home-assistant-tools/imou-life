...
The one-way TTS-to-camera-speaker path is now proven without `libCommonSDK.so`
for the tested public relay route. The original native dependency split into
clear layers:
...
| DHHTTP visualtalk start + talk-send frames | No | `scripts/imou_visualtalk.py` opens Cseq 0 and START TALK through the pure Python tunnel, then sends Python-generated interleaved DHAV frames. Real-device test returned Cseq 0/1 `200 OK` and sent 50 DHAV frames. |
...
| DHHTTP talk-enable handshake | Solved through pure Python relay tunnel: Cseq 0 and START TALK both return `200 OK`; Python sends DHAV frames after that. | Replace silence test with real TTS/AAC or G.711 compatibility matrix per camera model. |
