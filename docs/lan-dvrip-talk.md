...
Imou app, no cloud, no P2P. This targets Dahua-family Imou cameras that are
physically reachable on the same LAN as the bridge.
...
- The app's account cameras, such as "remote camera" serial `EXAMPLESERIAL01`,
  are **remote** and are reached by
...
- The Imou/Alibaba **cloud is only used for signaling/control** (`47.236.x:443`,
  `8.214.105.88:8883` IoT signaling endpoint) — **media and talk do not flow through Imou's cloud**.
- So the app path is **P2P, not cloud-relayed**. The cloud only does rendezvous.

Consequence: LAN cameras are a separate concern from the app's remote cameras.
A bridge host on the same camera subnet can talk to them **directly**, skipping
P2P and cloud entirely.

## DVRIP login — WORKING

`scripts/imou_dvrip.py` logs in to `<camera-ip>:37777` and runs JSON-RPC, with
