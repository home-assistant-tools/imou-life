...
- **MQTT/IoT**: `cm_getTalkTransferStreamUrl` via `iotManager.doServiceSync(...)`.
...
Captured on the real S21 against camera "Cam sân" (serial `EXAMPLESERIAL01`,
on the LAN) by instrumenting the app with frida-gadget and hooking the talk
classes. Setup: gadget injected as `DT_NEEDED` of `libCommonSDK.so` in the
