...
**hooking the running app** (Frida on the patched APK, test Android device). No network MITM needed.
...
our voice. **Verified: the "example living-room camera" camera spoke an injected TTS clip.**
...
3. **Test on an audible camera.** Best target: an indoor account camera with a working
   speaker that is P2P-reachable (e.g. "example living-room camera"), with someone nearby to
   listen.
