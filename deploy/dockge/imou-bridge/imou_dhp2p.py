...
class PTCPSession:
    ...
    def observe(self, packet: PTCPPacket) -> PTCPPacket:
        self.last_duplicate = False
        packet_end = packet.sent + packet.body.wire_len
        if packet.body.kind is not BodyKind.EMPTY and packet_end <= self.recv:
            self.last_duplicate = True
        else:
            self.recv = max(self.recv, packet_end)
...
def format_payload_preview(data: bytes, limit: int = 600) -> str:
    preview = data[:limit]
...
async def p2p_handshake(
    ...
    *,
    ...
    encrypted_addr = str(device_response.body.get("IpEncrpt") or device_response.body.get("body/IpEncrpt") or "").lower()
    if dtype > 0 and encrypted_addr != "false":
        assert username is not None and password is not None
        key = device_login_key(username, password, rand_salt)
...
class DHP2PTunnel:
    ...
    async def _accept_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            ...
            if first_data.done() and first_data.result():
                if self.debug:
                    print(f"RTSP >>> realm={realm:08x} {format_payload_preview(first_data.result())}")
    ...
    async def _client_to_ptcp_with_first(
        ...
        if not data:
            return
        if self.debug:
            print(f"RTSP >>> realm={realm:08x} {format_payload_preview(data)}")
    ...
    async def _ptcp_reader(self) -> None:
        while True:
            ...
            if body.kind is BodyKind.HEARTBEAT:
                continue
            if self.ptcp.session.last_duplicate:
                if self.debug:
                    print(f"PTCP duplicate skipped {format_ptcp_packet(packet)}")
                continue
            if body.kind is BodyKind.STATUS:
                if self.debug:
                ...
                if self.debug:
                    print(f"RTSP <<< realm={body.realm:08x} {format_payload_preview(body.data)}")
...
async def amain() -> int:
    ...
    parser.add_argument("--password", default=os.environ.get("IMOU_DHP2P_PASSWORD"), help="camera password for type > 0")
